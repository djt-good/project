...
//         }
    ...
    function condiction() {
        ...
        $.ajax({
            type:"post",
            url:"${pageContext.request.contextPath}/pro/splitajax",
...
<div id="brall">
    ...
    <div id="condition" style="text-align: center">
        <form id="myform">
            ...
            <input type="button" value="查询" onclick="condiction()">
        </form>
    </div>
    <br>
...
<script type="text/javascript">
    ...
    function deleteBatch() {
           <%--var zhi = $("input[name=ck]:checked").length;--%>
           <%--var str = "";--%>
           <%--var id = "";--%>
           <%--if (zhi == 0){--%>
           <%--    alert("请选中要删除的商品");--%>
           <%--}else {--%>
           <%--    if (confirm("你确定要删除"+zhi+"条商品么？")){--%>
           <%--          // id = $(item).val();--%>
           <%--          // if (id !=null){--%>
           <%--          //     id +="";--%>
           <%--          //     str=id--%>
           ...
           <%--      --%>
           <%--        })--%>
           <%--        &lt;%&ndash;window.location="${pageContext.request.contextPath}/prod/deletebeach?str="+str;&ndash;%&gt;--%>
           <%--   --%>
           <%--        $.ajax({--%>
           <%--            url: "${pageContext.request.contextPath}/pro/deletebeach",--%>
           <%--            data: {"pids":str},--%>
           <%--            type: "post",--%>
           <%--            dataType:"text",--%>
           ...
           <%--}--%>
            ...
            if(zhi.length==0){
                ...
                //拼接ID
                    ...
                    //发送请求到服务器端
                 <%--window.location="${pageContext.request.contextPath}/pro/deletebeach?pids"+pids;--%>
                    $.ajax({
                                url: "${pageContext.request.contextPath}/pro/deletebeach",
                                data: {"pids":pids},
                                type: "post",
    ...
    function del(pid,page) {
        ...
        if (confirm("确定删除吗")) {
          ...
          //向服务器提交请求完成删除
          $.ajax({
              url: "${pageContext.request.contextPath}/pro/delete",
            ...
            <%--  window.location="${pageContext.request.contextPath}/pro/delete?pid="+pid;--%>
        }
    }
    ...
    function one(pid, page) {
        ...
        location.href = "${pageContext.request.contextPath}/pro/one" + str;
    }
</script>
<!--分页的AJAX实现-->
<script type="text/javascript">
    function ajaxsplit(page) {
       ...
       $.ajax({
           url:"${pageContext.request.contextPath}/pro/splitajax",
           data:{"page":page,"pname":pname,"typeid":typeid,"lprice":lprice,"hprice":hprice},//,"pname":pname,"typeid":typeid,"lprice":lprice,"hprice":hprice
           type: "post",
           success:function () {
           $("#table").load("http://localhost:8080/admin/product.jsp #table")
           }
